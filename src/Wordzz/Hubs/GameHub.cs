...
namespace NBsoft.Wordzz.Hubs
{
    public class GameHub : Hub
    {
        ...
        public GameHub(IGameService gameService, IGameQueueService gameQueueService)
        {
        ...
        public Task<IEnumerable<IGameChallenge>> GetReceivedChallenges()
        {
            ...
            var queued = gameQueueService.GetReceivedChallenges(player.UserName);
            var challenges = queued.Select(q => new GameChallenge { Id = q.Id, Language = q.Language, BoardId = q.BoardId, Origin = q.Player1, Destination = q.Player2 } as IGameChallenge);
        ...
        public Task<IEnumerable<IGameChallenge>> GetSentChallenges()
        {
            ...
            var queued = gameQueueService.GetSentChallenges(player.UserName);
            var challenges = queued.Select(q => new GameChallenge { Id = q.Id, Language = q.Language, BoardId = q.BoardId, Origin = q.Player1, Destination = q.Player2 } as IGameChallenge);
            return Task.FromResult(challenges);
        }
        ...
        public async Task<IGameQueue> SearchGame(string language, int boardId)
        {
            var user = ClientHandler.Find(Context.ConnectionId);
            if (user == null || user.UserName == null)
                return null;

            return await gameService.SearchGame(language, boardId, user.UserName);
        }
        public async Task<IGameChallenge> ChallengeGame(string language, int boardId, string challengedPlayer)
        {
            ...
            var challenge = await gameService.ChallengeGame(language, boardId, challenger.UserName, opposer.UserName);            
        ...
        public async Task<string> ChallengeAccept(string queueId, bool accept)
        {
            ...
            var queue = gameQueueService.GetQueue(queueId);
            var game = await gameService.AcceptChallenge(queueId, accept);
            if (queue == null){
                return null;
            }
