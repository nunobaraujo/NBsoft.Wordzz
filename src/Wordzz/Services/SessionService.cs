...
namespace NBsoft.Wordzz.Services
{
    internal class SessionService : ISessionService
    {
        ...
        {
            var dbSessions = await sessionRepository.List();
        ...
        public async Task<ISession> GetSession(string sessionToken)
        {
            ...
            var session = await sessionRepository.Get(sessionToken);
            if (session == null)
                return null;
            ...
            if (DateTime.UtcNow > session.Registered.AddMilliseconds(SessionMaxAge))
            {
                ...
                await log.InfoAsync($"Session expired: [{session.SessionToken}]", context: session.UserId);
                return null;
            }
            ...
            {
                await sessionRepository.Remove(sessionToken);                
                await log.InfoAsync($"Session timeout: [{session.SessionToken}]", context: session.UserId);
                return null;
            }

            await sessionRepository.Touch(sessionToken, DateTime.UtcNow);
        ...
        public async Task<ISession> LogIn(string userName, string password, string userInfo)
        {
            ...
            if (userName == Constants.AdminUser && password == Constants.AdminPassword)
                user = (await userRepository.Get(Constants.AdminUser));
            else
                user = await userRepository.Auth(userName, password);
            ...
            var key = Encoding.ASCII.GetBytes(settings.EncryptionKey);
        ...
        public async Task LogOut(string sessionToken)
        {
            ...
            try
            {
                ...
                {
                    await sessionRepository.Remove(sessionToken);
                    await log.InfoAsync($"Session Terminated: [{session.SessionToken}]", context: session.UserId);
                }
            }
            catch (Exception ex)
            {   
                await log.ErrorAsync(ex.Message, ex, context: sessionToken);
                throw;
            }
        }
        ...
        private async Task<ISession> CreateNewSession(string userId, string userInfo, string token)
        {
            ...
            try
            {
                ...
                await sessionRepository.New(newSession);                
                await log.InfoAsync($"New session: [{newSession.SessionToken}]", context: newSession.UserId);
                return newSession;
            }
            catch (Exception ex)
            {   
                await log.ErrorAsync(ex.Message, ex, context: userId);
                throw;
            }
        }
