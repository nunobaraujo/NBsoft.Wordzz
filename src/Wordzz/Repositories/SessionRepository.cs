...
namespace NBsoft.Wordzz.Repositories
{
    class SessionRepository : ISessionRepository
    {
        ...
        public async Task Touch(string session, DateTime date)
        {
            try
            {
                if (session == null)
                ...
                var query = $"UPDATE Session SET LastAction = @date  WHERE SessionToken=@session";

                using var cnn = _createdDbConnection();
                var res = await cnn.ExecuteScalarAsync<int>(query, new { session, date });
